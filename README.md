...
| `tools/mutate.py` | Restores each bug a guard claims to catch and confirms the guard goes red, judging by exit code. `tools/mutations.json` holds 38 mutations and 2 controls — a control is a change that must NOT matter, so a clean sweep is falsifiable rather than assumed |
