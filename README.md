...
| `tools/validate_run.py` | Enforces eighteen integrity rules, including discovery, reachability, mode proof, HTML/JSON parity, component adoption, identity evidence, artifact freshness, and a complete evidence-derived unification strategy. `--stamp` writes `provenance.validation_gate` and clears the report's validation banner — the only thing that marks a report finished — and only on a pass |
